...
##                  "LEAVE_SEASON_OUT_PREDICTIONS_NEW_TEAM_AND_SEASON_gam_nb_obj", 
##                  "_", league,
##                  #"_", string_add, 
##                  "_Corners.Robj"),
##      pred.list.nb)
...
for (league in league.name[3]){
  ...
  # load(file=paste0(ifelse(bucketed, "BUCKETED_", ""),
  #             "LEAVE_SEASON_OUT_PREDICTIONS_NEW_TEAM_AND_SEASON_gam_nb_obj", 
  #             "_", league,
  #             #"_", string_add, 
  #             "_Corners.Robj"))
  ...
  for (year in 2009:2023){
    ...
    gam.obj <- gam(Corners ~ s(Score.Diff,k=score.df) + half_id + s(Minute.clean, by=half_id) + s(Weighted.Win.Prob)  + HomeAway + s(RedCard.Diff, k=redcard.df)
                   + s(Team, bs="re") + s(season, bs="re"),
                   family = "poisson",  data = our.df.cut %>% mutate(Team = factor(as.character(Team)),
    ...
    incl.ind <- which(!grepl("\\(Team\\)", names(coef(gam.obj))) & !grepl("\\(season\\)", names(coef(gam.obj))))
    n.included.vars <- length(incl.ind)

    b <- coef(gam.obj)[incl.ind]
    Xp <- predict(gam.obj,
    ...
    save(file=paste0(ifelse(bucketed, "BUCKETED_", ""),
                     "LEAVE_SEASON_OUT_PREDICTIONS_NEW_TEAM_AND_SEASON_gam_obj",
                     "_", league,
                     #"_", string_add,
                     "_Corners.Robj"),
         pred.list)
    ...
    gam.nb.obj <- gam(Corners ~ s(Score.Diff,k=score.df) + half_id + s(Minute.clean, by=half_id) + s(Weighted.Win.Prob)  + HomeAway + s(RedCard.Diff, k=redcard.df)
                      + s(Team, bs="re") + s(season, bs="re"),
                      family = "nb", data = our.df.cut %>% mutate(Team = factor(as.character(Team)),
                                                                  season = factor(as.character(season))),
                      subset = (season != year))
