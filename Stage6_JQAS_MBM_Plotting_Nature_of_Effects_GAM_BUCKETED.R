#######
##  Stage 6: Code yielding Figure 3 of the main manuscript,
...
##           for the case of
##           SMOOTHING SPLINES WITH BUCKETED EXTREME CATEGORIES
##           (score diff +-3, red card diff +-2, game minute 45+)
#######
...
league <- "Bundesliga"
load(paste0("Fitted_Objects/GAM_W_BUCKETED_EXTREME_CATEGORIES_gam_nb_obj_Team_AND_Season_Mixed_Eff_",
...
for (league in league.name[-1]){
  ...
  all.smooths[[league]] <- ns.obj[[league]]
  rm(ns.obj)
}
...
adjusted_plots_shots <- lapply(seq_along(plots), function(i) {
  ...
  if (i %in% c(1,2)){
    plot + 
      labs(y = "Shots/Minute", x= paste0("Minute, ", ifelse(i==1, "1st", "2nd"), " half")) +
      scale_x_continuous(breaks = c(5,15,25,35,45),
                         labels=c("5", "15", "25", "35", "45+")) +  # Set custom breaks on x-axis
    ...
    plot + 
      labs(y = "Shots/Minute", x= "Red Card Difference") +
      # geom_vline(xintercept = c(-2:2), linetype = "dashed", color = "black") +
      coord_cartesian(ylim = y_limits) + 
      
     #  scale_x_continuous(breaks = -2:2) +  # Set custom breaks on x-axis
      scale_x_continuous(breaks = -2:2,
                       labels=c("-2\nor more", "-1", "0", "+1", "+2\nor more")) +  # Set custom breaks on x-axis

      theme_minimal() +
      theme(plot.title = element_blank())
  } else if (i == 4) {
    plot + 
      labs(y = "Shots/Minute", x= "Score Difference") +
     # geom_vline(xintercept = c(-2:2), linetype = "dashed", color = "black") +
      coord_cartesian(ylim = y_limits) + 
    ...
    plot +
      labs(y = "Shots/Minute", x= ifelse(i==1, "Minute", "Win Probability Difference")) +
      coord_cartesian(ylim = y_limits) + 
      theme_minimal() +
      theme(plot.title = element_blank())
  } 
...
adjusted_plots_shots_short1 <- adjusted_plots_shots[c(4,3,6)]
adjusted_plots_shots_short2 <- adjusted_plots_shots[c(1, 2)]
...
adjusted_plots_corners <- lapply(seq_along(plots), function(i) {
  ...
  if (i %in% c(1,2)){
    plot + 
      labs(y = "Corners/Minute", x= paste0("Minute, ", ifelse(i==1, "1st", "2nd"), " half")) +
      scale_x_continuous(breaks = c(5,15,25,35,45),
                         labels=c("5", "15", "25", "35", "45+")) +  # Set custom breaks on x-axis
    ...
    plot + 
      labs(y = "Corners/Minute", x= "Red Card Difference") +
      # geom_vline(xintercept = c(-2:2), linetype = "dashed", color = "black") +
      coord_cartesian(ylim = y_limits) + 
    ...
    plot + 
      labs(y = "Corners/Minute", x= "Score Difference") +
      # geom_vline(xintercept = c(-2:2), linetype = "dashed", color = "black") +
      coord_cartesian(ylim = y_limits) + 
      scale_x_continuous(breaks = -3:3,
                         labels=c("-3\nor more", "-2", "-1", "0", "+1", "+2", "+3\nor more")) +  # Set custom breaks on x-axis
    ...
    plot +
      labs(y = "Corners/Minute", x= ifelse(i==1, "Minute", "Win Probability Difference")) +
      coord_cartesian(ylim = y_limits) + 
      theme_minimal() +
      theme(plot.title = element_blank())
  } 
...
## For the slides presentation
adjusted_plots_corners_short1 <- adjusted_plots_corners[c(4,3,6)]
adjusted_plots_corners_short2 <- adjusted_plots_corners[c(1,2)]
